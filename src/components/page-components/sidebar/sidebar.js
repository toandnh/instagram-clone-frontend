...
const Sidebar = () => {
	...
	const toggleDrawerSearch = () => {
		...
		if (notiOpened) {
			...
			if (searchOpened) setNotiOpened(!searchOpened)
		}
	}
	...
	const toggleDrawerNoti = () => {
		...
		if (searchOpened) {
			...
			if (notiOpened) setSearchOpened(!notiOpened)
		}
	}

	const handleCreateOpen = () => {
		dispatch(setCreateOpened(true))
	}
					...
					<List sx={{ display: 'flex', flexDirection: 'column', flexGrow: '6', gap: '0.5rem' }}>
						...
						<ListItem disablePadding>
							<ListItemButton sx={style.button} onClick={toggleDrawerSearch}>
								<Tooltip
									title={drawerOpened || searchOpened ? '' : 'Search'}
									placement='right'
									arrow
									ref={searchRef}
								>
						...
						<ListItem disablePadding>
							<ListItemButton sx={style.button} onClick={toggleDrawerNoti}>
								<Tooltip
									title={drawerOpened || notiOpened ? '' : 'Notifications'}
									placement='right'
									arrow
									ref={notiRef}
								>
